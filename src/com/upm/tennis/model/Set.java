...
public class Set {
  ...
  public Player getSetWinner() {
    ...
    Map<Player, Integer> gameWinsByPlayer = getGameWinsByPlayer();
    if (gameWinsByPlayer.isEmpty()) {
      return null;
    }

    if (gameWinsByPlayer.size() == 1) {
      return this.getWinnerFromSinglePlayerWithGamePoints(gameWinsByPlayer);
    }
    return this.getGameWinnerFromTwoPlayersWithGamePoints(gameWinsByPlayer);
  }

  private Player getWinnerFromSinglePlayerWithGamePoints(Map<Player, Integer> pointWinsByPlayer) {
    ...
    return this.hasPlayerWonSet(playerEvaluated) ? playerEvaluated.getKey() : null;
  }

  private Player getGameWinnerFromTwoPlayersWithGamePoints(Map<Player, Integer> pointWinsByPlayer) {
    Iterator<Entry<Player, Integer>> iterator = pointWinsByPlayer.entrySet().iterator();
    Entry<Player, Integer> playerEvaluatedA = iterator.next();
    Entry<Player, Integer> playerEvaluatedB = iterator.next();

    if (hasPlayerWonSet(playerEvaluatedA, playerEvaluatedB.getValue())) {
      return playerEvaluatedA.getKey();
    }

    if (hasPlayerWonSet(playerEvaluatedB, playerEvaluatedA.getValue())) {
  ...
  public Player getRestPlayerFromFirstPointInLastFinishedGame() {
    ...
    return (lastFinishedGame != null) ? lastFinishedGame.getRest() : null;
  }
  ...
  public Player getNexServicePlayerFromCurrentGame() {
    ...
    if (currentGame != null) {
      return currentGame.getNextServicePlayerFromNonFirstPointInGame();
    }

    Game lastPlayed = this.getLastFinishedGame();
    if (lastPlayed != null) {
      return lastPlayed.getRest();
    }
    return null;
  }
